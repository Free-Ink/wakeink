...
/* Perform extra checks on the input stream, even if they aren't proven
   ...
   crashes). */
/* WakeInk: ON. A truncated/desynced deflate stream (e.g. a slow TCP read
   surfacing as a short EOF) otherwise yields out-of-range Huffman symbols
   that drive wild dictionary offsets and corrupt memory. With this set, the
   decoder returns TINF_DATA_ERROR instead of faulting. */
